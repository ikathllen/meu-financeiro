document.addEventListener('DOMContentLoaded', () => {
  ...
  // ⚠️ Função corrigida: não subtrai mais um mês!
  function getLabelAndYear(ym) {
    const dateSel = parseYearMonth(ym);
    ...
    return {
      ...
      key: `${dateSel.getFullYear()}-${String(dateSel.getMonth() + 1).padStart(2, '0')}`
    };
  }
  ...
  function renderAll() {
    const userVal = mInput.value;
    const { labelMonth, year, key } = getLabelAndYear(userVal);
    ...
    if (isAn) {
      months.forEach(mo => {
        ...
        if (exp > 0) {
          const dDate = parseYearMonth(mo);
          const lab = `${dDate.toLocaleString('pt-BR',{month:'long'})} de ${dDate.getFullYear()}`;
    ...
    chart = new Chart(chartEl, {
      ...
      data: {
        ...
        datasets: [{ data: vals, backgroundColor: [
          '#DAF0F7','#D5E3F0','#C8D9F0','#C1D5F0','#B4CBF0','#A8B5E0',
          '#E1F3FB','#CDEAF6','#ABE7FE','#8CE1F4','#AEC6CF','#79D2E6'
        ]}]
      },
